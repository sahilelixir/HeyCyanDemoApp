...
- (void)scanWithTimeout:(NSInteger)timeout {
    ...
    if([connectedP count] > 0) {
        ...
        if(self.delegate && [self.delegate respondsToSelector:@selector(didScanPeripherals:)]) {
            NSLog(@"Calling didScanPeripherals (initial) with %lu devices", (unsigned long)[self.peripherals count]);
            [self.delegate didScanPeripherals:self.peripherals];
        } else {
            NSLog(@"WARNING: delegate not set or doesn't respond to didScanPeripherals (initial)!");
        }
    }
...
//    }
    ...
    if(self.delegate && [self.delegate respondsToSelector:@selector(didScanPeripherals:)]) {
        NSLog(@"Calling didScanPeripherals with %lu devices", (unsigned long)[self.peripherals count]);
        [self.delegate didScanPeripherals:self.peripherals];
    } else {
        NSLog(@"WARNING: delegate not set or doesn't respond to didScanPeripherals!");
    }
}
...
// State restoration is not used in this app
/*
